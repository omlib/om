...
abstract Version(SemVer) from SemVer to SemVer {
    ...
    @:from public static function stringToVersion( s : String ) {
        if( !VERSION.match(s) )
            throw 'Invalid SemVer format for "$s"';
    ...
    inline function new( major : Int, minor : Int, patch : Int,
                         pre : Array<Identifier>, build : Array<Identifier> )
        this = {
  ...
  static inline function sanitize( s : String ) : String return SANITIZER.replace(s,'');
}

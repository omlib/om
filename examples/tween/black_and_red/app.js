	...
	window.onload = function(_) {
		...
		while(_g < 64) {
			...
			while(_g1 < 64) {
				...
				td[0].style.background = "#000";
				var x = (i + j) * 0.1;
				var cell = [{ td : td[0], value : 0}];
				var tween = new om_Tween(cell[0]).to({ value : 1},8000).delay((0.001 * index + Math.random()) * 500).easing(om_easing_Elastic.InOut).onUpdate((function(cell,td) {
					return function() {
						var c = Math.floor(cell[0].value * 255);
						td[0].style.background = "rgb(" + c + ", 0, 0)";
					};
				})(cell,td));
				var tweenBack = new om_Tween(cell[0]).to({ value : 0},4000).delay((0.001 * index + Math.random()) * 500).easing(om_easing_Elastic.InOut).onUpdate((function(cell,td) {
					return function() {
						var c1 = Math.floor(cell[0].value * 255);
						td[0].style.background = "rgb(" + c1 + ", 0, 0)";
					};
				})(cell,td));
